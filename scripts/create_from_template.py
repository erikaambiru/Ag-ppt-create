...
def add_image_to_slide(slide, prs, image_config: dict, content_path: str) -> dict:
    ...
    try:
        if position == 'full' or position == 'center':
            ...
            img_left = Inches(0.5)
            img_top = Inches(1.5)
            img_width = Inches(content_width * width_pct / 100) if position == 'center' else Inches(content_width)
            # Center the image if position is 'center'
            if position == 'center':
                img_left = Inches((slide_width_inches - (content_width * width_pct / 100)) / 2)
...
def add_slide_from_layout(prs, layout_idx: int, title: str, items: Optional[list] = None, 
                          ...
                          content_path: Optional[str] = None, code: Optional[str] = None) -> None:
    ...
    if subtitle:
        ...
        if subtitle_placeholder and subtitle_placeholder.has_text_frame:
            ...
            # This is especially important for section slides
            slide_width = prs.slide_width.inches
            slide_height = prs.slide_height.inches
            
            # Position subtitle below the title (around 35-40% down)
            subtitle_box = slide.shapes.add_textbox(
                Inches(0.5), 
                Inches(slide_height * 0.38),  # Below title
                Inches(slide_width - 1),       # Almost full width
    ...
    if items:
        ...
        # No items provided - remove empty body placeholders to reduce whitespace
...
def create_pptx_from_template(template_path: str, content_path: str, output_path: str, config_path: Optional[str] = None, force: bool = False, add_signature: bool = True) -> None:
    ...
    for i, slide_data in enumerate(slides_data):
        ...
        if layout_idx is None:
            ...
            # If photo slide has no items, prefer photo-specific layout
            elif slide_type == 'photo' and not items and has_image:
                photo_layout = layout_map.get('photo') or layout_map.get('blank')
        ...
        slide = add_slide_from_layout(prs, layout_idx, title, items, subtitle, image_config, content_path, code)
...
'''
    ...
    create_pptx_from_template(
        args.template, args.content, args.output, config_path,
